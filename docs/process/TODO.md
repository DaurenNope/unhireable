...
- [x] **1. PDF CV Generator** ✅ DONE
  - **What:** Auto-generates ATS-optimized PDF CVs tailored to jobs
  - **Files:** `cv-generator/generate-cv.mjs`, `cv-generator/package.json`
  - **Features:**
    - [x] ATS-optimized HTML template (clean, simple, scannable)
    - [x] Job-specific tailoring with highlighted skills
    - [x] Generated via Puppeteer (headless Chrome)
    - [x] Letter size (US) format
    - [x] Per-job CV generation: "📄 Generate CV" button on each APPLY job card
    - [x] Batch generation: "📄 Generate All CVs" button in dashboard header
  - **API Endpoints:**
    - POST /api/cv/generate - Generate single job CV
    - POST /api/cv/generate-all - Generate all APPLY recommendation CVs
  - **Output:** `data/generated-cvs/` with `cv-manifest.json`
