...
const SAMPLE_JOBS: Job[] = [
  {
    ...
    description:
      'We are looking for an experienced frontend developer to join our remote-first team.',
    requirements:
      'Expertise with React, TypeScript, Tailwind CSS. Experience with state machines a plus.',
    location: 'Remote',
    salary: '$120k - $180k',
    source: 'demo',
    status: 'saved',
  ...
  },
  {
    ...
    description:
      'Own features end-to-end across our Node/React stack. Work directly with founders.',
  ...
  },
  {
    ...
    description:
      'Help us scale our data platform to millions of events per minute.',
  ...
  },
  {
    ...
    description:
      'Build resilient infrastructure with Terraform and Kubernetes.',
  ...
  },
  {
    ...
    description:
      'Create beautiful mobile experiences for millions of users across iOS and Android.',
  ...
  },
  {
    ...
    description:
      'Design delightful user experiences for our flagship product. Collaborate with product and engineering.',
  ...
  },
  {
    ...
    description:
      'Build reliable automation suites for our web and mobile applications.',
...
const FALLBACK_JOB: Job = SAMPLE_JOBS[0] ?? {
  id: 0,
  title: 'Sample Role',
  ...
  try {
    ...
    if (stored) {
      ...
      memoryDb = {
        ...
        nextJobId:
          parsed.nextJobId ?? (parsed.jobs?.length ?? SAMPLE_JOBS.length) + 1,
...
function generateMatchScore(job: Job, profile?: UserProfile): number {
  const base = Math.random() * 20 + 60 // 60 - 80 baseline
  const bonus =
    profile?.skills?.technical_skills?.some((skill) =>
...
function computeMarketInsights(
  ...
  const totalPrevious = Math.max(0, Math.round(total as number * 0.75))
  ...
  const trendingSkills = topStats(skillCounts, total, 12) as SkillStat[]

  const userSkills = new Set<string>()
  if (db.profile) {
    db.profile.skills.technical_skills.forEach((skill) =>
      userSkills.add(skill.toLowerCase())
    )
    db.profile.experience.forEach((exp) =>
      exp.technologies.forEach((tech) => userSkills.add(tech.toLowerCase()))
    )
  }

  const skillsToLearn = Object.entries(skillCounts)
    .filter(([skill]) => !userSkills.has(skill.toLowerCase()))
    .map<SkillStat>(([name, count]) => ({
      name,
      job_count: count,
      percentage: percentage(count, total),
    }))
  ...
  return {
    ...
    skill_trends: trendingSkills.slice(0, 8).map((stat) => {
      const previous = Math.max(0, Math.round(stat.job_count as number * 0.75))
      return {
        name: stat.name,
    ...
    role_trends: topRoles.map((stat) => {
      const previous = Math.max(0, Math.round(stat.job_count as number * 0.8))
      return {
        name: stat.name,
        current_count: stat.job_count,
        previous_count: previous,
...
export const fallbackSchedulerConfig: SchedulerConfig = {
  enabled: false,
  schedule: '0 9 * * *',
  query: 'demo',
  sources: [],
...
export const fallbackSchedulerStatus: SchedulerStatus = {
  enabled: false,
  running: false,
  schedule: '0 9 * * *',
  query: 'demo',
...
export async function handleMockCommand(
  ...
  switch (command) {
    ...
    case 'update_job_match_scores': {
      const profile = args?.profile as UserProfile | undefined
    ...
    default:
      throw new Error(
        `Tauri bridge not available. Command "${command}" is not supported in web preview mode.`
      )
  }
}
