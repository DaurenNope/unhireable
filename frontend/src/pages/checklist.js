...
export default function ChecklistPage() {
    ...
    if (loading) {
        return (_jsx("div", { className: "flex items-center justify-center h-full", children: _jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary" }) }));
    }
    return (_jsxs("div", { className: "p-6 max-w-4xl mx-auto", children: [_jsx("h1", { className: "text-2xl font-bold mb-6", children: "Development Checklist" }), _jsx("div", { className: "space-y-4", children: checklist.map((task, index) => (_jsxs("div", { className: "flex items-start", children: [_jsx("input", { type: "checkbox", checked: task.completed, onChange: () => toggleTask(index), className: "mt-1 h-5 w-5 rounded border-gray-300 text-primary focus:ring-primary" }), _jsx("span", { className: `ml-3 ${task.completed ? 'line-through text-gray-500' : 'text-gray-900'}`, children: task.text })] }, index))) })] }));
}
