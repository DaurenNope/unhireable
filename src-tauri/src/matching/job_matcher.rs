...
use crate::matching::{skills_analyzer::SkillsAnalyzer, JobMatchResult};
...
impl JobMatcher {
    ...
    fn calculate_location_match(&self, job: &Job, profile: &UserProfile) -> f64 {
        ...
        for job_word in &job_words {
            for user_word in &user_words {
                if job_word.len() > 3 && user_word.len() > 3 && job_word.contains(user_word)
                    || user_word.contains(job_word)
                {
                    return 75.0;
                }
            }
    ...
    fn calculate_title_match(&self, job: &Job, profile: &UserProfile) -> f64 {
        ...
        let user_experience: Vec<String> = profile
            .experience
            .iter()
    ...
    fn generate_match_reasons(&self, result: &JobMatchResult) -> Vec<String> {
        ...
        if result.skills_match >= 80.0 {
            reasons.push(format!(
                "Excellent skills match: {}% of required skills",
                result.skills_match as u32
            ));
        } else if result.skills_match >= 60.0 {
            reasons.push(format!(
                "Good skills match: {}% of required skills",
                result.skills_match as u32
            ));
        } else if result.skills_match >= 40.0 {
            reasons.push(format!(
                "Fair skills match: {}% of required skills",
                result.skills_match as u32
            ));
        } else {
            reasons.push(format!(
                "Limited skills match: {}% of required skills",
                result.skills_match as u32
            ));
        }
